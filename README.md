Script de Manutenção e Limpeza Automatizada (SecOps)  
    Este repositório contém uma ferramenta de automação desenvolvida em Python para a manutenção proativa de diretórios temporários. O foco principal é a eficiência operacional e a redução de riscos, garantindo que arquivos antigos sejam removidos de forma segura e auditável.  
   ...
   🛠️ Tecnologias UtilizadasLinguagem: Python 3.12+Linters/Formatadores: Ruff e Black (Garantia de padronização PEP 8)Security Scan: Bandit (Análise estática de vulnerabilidades no código)